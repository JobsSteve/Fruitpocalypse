...
-(void)makePages {
	...
	for (int i=0; i < [arrayPages count]; i++) {
        ...
        //n.position = ccp(s.width  + i * s.width, s.height );
		n.position = ccp(s.width / 2 + i * s.width, s.height / 2);
		[world addChild:n];
	}
	//world.anchorPoint = ccp(0,0);
    //world.position = ccp(-s.width -s.width * currentPage, -s.height);
...
- (BOOL)containsTouchLocation:(UITouch *)touch
{
	...
	CGRect r = CGRectMake(-self.contentSize.width, -self.contentSize.height , self.contentSize.width , self.contentSize.height);
...
- (void) moveToPagePosition {
	...
	CGSize s = self.contentSize;
	//float diffX = fabs( (positionNow.x) - (-s.width -s.width * currentPage) );
    float diffX = fabs( (positionNow.x) - (-s.width /2 -s.width * currentPage) );
    //	CCLOG(@"diff[%f]",diffX);
	
	if (diffX > 0) {
		//id moveTo = [CCMoveTo actionWithDuration:( diffX / s.width)  position:ccp(-s.width  -s.width * currentPage, -s.height)];
